...
{
    public partial class gzjhtj : Form
    {
        string GuZiID = "";
        public gzjhtj()
        {
                ...
                if (c == DialogResult.OK)
                {
                    ...
                    else
                    {
                        ...
                        if (panduan1.Rows.Count > 0)
                        {
                            ...
                            string sql = $"select * from GudinZiChan where GuZiID ='{GuZiID}'";
                            DataTable panduan = DBHelper.GetDataTable(sql);
                            if (panduan.Rows.Count > 0)
                            {
                                string sql1 = $"insert Jei(GuZiID,ZuJieGongSi,ZuJieShuLiang,JeiRiQi) values('{GuZiID}','{gs}','{sl}','{rq1}')";
                                DBHelper.GetDataTable(sql1);
                                string sql2 = $"update GudinZiChan set ZhuangTaiID = '3' where GuZiID ='{GuZiID}'";
                        ...
                        {
                            c=MessageBox.Show("该商品不存在" ,"", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                            if (c == DialogResult.OK)
                            {
                                lzm.GudinZiChanyianjia GudinZiChanyianjia = new lzm.GudinZiChanyianjia();
